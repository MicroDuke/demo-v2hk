...
def wb_init ():
    ...
    #COLUMN=['日期'	'买交易量B1'	'卖交易量S1'	'......' '买交易量Bn'	'卖交易量Sn'	'BM1'	'SM1' '......'	'BMn'	'SMn']
    COLUMN=[]
    COLUMN.insert(0,'文件名')
    ...
    for id in range(0,len(NUMBER)) :
        ...
        COLUMN.insert(2*len(NUMBER)+2*id+3 , 'BM[' + str(NUMBER[id]) + ']')
        COLUMN.insert(2*len(NUMBER)+2*id+4 , 'SM[' + str(NUMBER[id]) + ']')
    ...
    COLUMN.insert(4*len(NUMBER)+3 , 'BM[>' + str(NUMBER[-1]) + ']')
    COLUMN.insert(4*len(NUMBER)+4 , 'SM[>' + str(NUMBER[-1]) + ']')
...
def ParLine (lines):
    ...
    for line in lines:
        ...
        if len(str) == 5 :
            ...
            BM_index =   2 * priceIndex + 3 + 2 * len(NUMBER)
            SM_index =   2 * priceIndex + 4 + 2 * len(NUMBER)
            ...
            if  re.search("B", line):
                ...
                #print('B_index,BM_data',B_index,BM_data)
