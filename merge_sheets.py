...
def merge_excel_sheets(merged_file, tobe_merged):
    ...
    sheet = wb_merged.create_sheet('Sheet1')
    newrowx = 0
    for sheet_name,sheet_data in wb_merged_dict.items():
        ...
        #sheet = wb_merged.add_sheet('Sheet1')
        for rowx in range(len(sheet_data)):
            #print("rowx,newrowx",rowx,newrowx)
            for colx in range(len(sheet_data[rowx])):
                #print("rowx,colx,",rowx,colx)
                sheet.cell(row=newrowx+1, column=colx+1, value=sheet_data[rowx][colx])
            newrowx = newrowx + 1
